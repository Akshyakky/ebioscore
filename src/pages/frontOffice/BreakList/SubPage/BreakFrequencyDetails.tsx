...
const BreakFrequencyDetails: React.FC<BreakFrequencyDetailsProps> = ({ open, onClose, endDateFromBreakDetails, onSave, initialFrequencyData }) => {
  ...
  const handleWeekDaysChange = (day: string, checked: boolean) => {
    setFrequencyData((prev) => ({
      ...prev,
      weekDays: checked ? [...prev.weekDays, day] : prev.weekDays.filter((d) => d !== day),
    }));
  };
  ...
  return (
    <GenericDialog
      ...
      actions={
        <>
          <CustomButton variant="contained" text="Save" onClick={handleSave} size="small" icon={Save} />
          <CustomButton variant="outlined" text="CLOSE" onClick={onClose} icon={Close} size="small" />
        </>
      }
    >
      <FormField
