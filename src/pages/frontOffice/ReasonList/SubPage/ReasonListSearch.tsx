...
const ReasonListSearch: React.FC<ReasonListSearchProps> = ({
  ...
  useEffect(() => {
    if (show) {
      performSearch(""); // Fetch all reasons when the form is opened
    }
  }, [show]);
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      performSearch(""); // Fetch all reasons when the search term is cleared
    }
  ...
  const handleSwitchChange = async (reason: ReasonListData, checked: boolean) => {
    try {
      ...
      if (result.success) {
        ...
        setSwitchStatus((prevState) => ({
          ...prevState,
          [reason.arlID]: !checked, // revert to previous state
        }));
      }
    } catch (error) {
  ...
  const columns = [
    ...
    { key: "rlName", header: "Resource", visible: true }, // Ensure visibility
    { key: "rNotes", header: "Instructions", visible: true }, // Ensure visibility
    {
      key: "status",
      header: "Status",
      visible: true,
