...
const ReasonDetails: React.FC<ReasonDetailsProps> = ({
    ...
    const [activeLabel, setActiveLabel] = useState(reasonData.rActiveYN === 'Y' ? 'Active' : 'Hidden');
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6} md={3}>
                        <FloatingLabelTextBox
                            ...
                            onChange={(e) => setReasonData({ ...reasonData, arlCode: e.target.value })}
                            isMandatory
                            size="small"
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                        <FloatingLabelTextBox
                            ...
                            onChange={(e) => setReasonData({ ...reasonData, arlName: e.target.value })}
                            isMandatory
                            size="small"
                            isSubmitted={isSubmitted}
                            name="arlName"
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                        <TextField
                            ...
                            InputProps={{
                                endAdornment: (
                                    <InputAdornment position="end">
                                        <IconButton onClick={incrementDuration} edge="end">
                                        </IconButton>
                                        <IconButton onClick={decrementDuration} edge="end">
                                        </IconButton>
                                        <Typography
                                            variant="body2"
                            ...
                            size="small"
                            inputProps={{ min: 0 }} // Ensure no negative numbers
                        />
                    </Grid>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                        <TextArea
                            label="Instruction"
                            name="arlDurDesc"
                            value={reasonData.arlDurDesc}
                            onChange={(e) => setReasonData({ ...reasonData, arlDurDesc: e.target.value })}
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                        <DropdownSelect
                            label="Resource"
                            value={reasonData.rlID.toString()} // Convert number to string
                            onChange={(e) => setReasonData({ ...reasonData, rlID: parseInt(e.target.value) })} // Convert string to number
                            options={resourceList.map((resource) => ({
                                value: resource.rLID.toString(), // Convert number to string
