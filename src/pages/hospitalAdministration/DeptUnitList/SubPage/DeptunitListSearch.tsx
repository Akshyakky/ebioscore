...
const DeptUnitListSearch: React.FC<DeptUnitListSearchProps> = ({ open, onClose, onSelect }) => {
  const fetchItems = () => DeptUnitListService.getAllDeptUnitList().then((result) => result.data || []);

  const updateActiveStatus = async (id: number, status: boolean) => {
    const result = await DeptUnitListService.updateDeptUnitListActiveStatus(id, status);
