...
const ManageBedDetails: React.FC<ManageBedDetailsProps> = ({ onBedSelect, isSelectionMode = false }) => {
  ...
  const fetchRoomsAndBeds = async (roomGroupId: number) => {
    setLoading(true);
    try {
      const [roomListResult, bedsResult] = await Promise.all([roomListService.find(`rgrpID == ${roomGroupId}`), wrBedService.find(`RoomList.rgrpID == ${roomGroupId}`)]);
  ...
  const getBedStyles = useCallback(
  ...
  const filteredBeds = useCallback(
    (room: RoomListDto) => {
      const beds = bedsByRoom[room.rlID] || [];
      return beds
        .filter(
          (bed) =>
            ...
            (bed.bedName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) || room.rName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()))
        )
        .map((bed) => ({
          ...bed,
          roomList: {
    ...
    <Box sx={{ height: "calc(100vh - 64px)", display: "flex" }}>
      ...
      <Box sx={{ width: 250, borderRight: `1px solid ${theme.palette.divider}`, display: "flex", flexDirection: "column" }}>
        ...
        <Box sx={{ flexGrow: 1, overflowY: "auto" }}>
          <CustomTreeView data={roomGroupHierarchy} onNodeSelect={handleRoomGroupSelect} />
        </Box>
      </Box>
          ...
          <>
            ...
            <Box sx={{ p: 2, borderBottom: `1px solid ${theme.palette.divider}` }}>
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                <Typography variant="h6">Manage Bed for {selectedRoomGroup.rGrpName}</Typography>
                <CustomButton variant="outlined" text="Refresh" onClick={() => fetchRoomsAndBeds(selectedRoomGroup.rGrpID)} icon={RefreshIcon} />
              </Box>
                          ...
                          <Tooltip key={bed.bedID} title={`Status: ${bed.bedStatus}`}>
                            <Box
                              ...
                              sx={{
                                ...
                                "&:hover":
                                  isSelectionMode && bed.bedStatus === "Available"
                                    ? {
                                        opacity: 0.8,
