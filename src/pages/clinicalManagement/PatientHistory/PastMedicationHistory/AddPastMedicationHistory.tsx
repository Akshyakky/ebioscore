...
const AddPastMedicationHistory: React.FC<AddPastMedicationHistoryProps> = ({ pchartId, opipNo, opipCaseNo, onHistoryChange, showImmediateSave, pastMedicationData }) => {
  ...
  const handleInputChange = useCallback((index: number, field: keyof PastMedicationDetailDto, value: any) => {
    setPastMedications((prev) => prev.map((med, i) => (i === index ? { ...med, [field]: value } : med)));
  }, []);
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={3}>
          <CustomButton variant="contained" color="primary" text="Add Medication" onClick={handleAddMedication} disabled={!searchTerm} icon={AddIcon} sx={{ width: "100%" }} />
        </Grid>
        <Grid item xs={12}>
          <CustomGrid columns={columns} data={pastMedications} pagination={false} maxHeight="300px" />
        </Grid>
        {showImmediateSave && (
          <Grid item xs={3}>
            <CustomButton
