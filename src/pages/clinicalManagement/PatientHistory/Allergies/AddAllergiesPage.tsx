...
const AddAllergiesHistory: React.FC<AddAllergiesHistoryProps> = ({ pChartId, opipNo, opipCaseNo, onHistoryChange, showImmediateSave, allergiesData }) => {
  ...
  const handleRemoveAllergy = useCallback((medicationName: string) => {
    showAlert("Confirm Removal", "Are you sure you want to remove this allergy?", "warning", {
  ...
  const columns: Column<OPIPHistAllergyDetailDto>[] = [
    ...
    { key: "genericName", header: "Generic Name", visible: true },
    {
      ...
      header: "Actions",
      visible: true,
      render: (item) => <CustomButton variant="outlined" color="secondary" size="small" text="Remove" icon={DeleteIcon} onClick={() => handleRemoveAllergy(item.medicationName)} />,
    },
  ];
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={3}>
          <CustomButton variant="contained" color="primary" text="Add Allergy" onClick={handleAddAllergy} disabled={!searchTerm} icon={AddIcon} sx={{ width: "100%" }} />
        </Grid>
        <Grid item xs={12}>
          <CustomGrid columns={columns} data={allergies} pagination={false} maxHeight="300px" />
        </Grid>
        {showImmediateSave && (
          <Grid item xs={3}>
            <CustomButton
