...
const PatientHistorySection: React.FC<PatientHistorySectionProps> = ({ pChartID, opipNo, opipCaseNo, shouldClear, onHistoryChange, showImmediateSave, initialHistoryData }) => {
  const [activeHistoryTab, setActiveHistoryTab] = useState(0);
  const theme = useTheme();

  const [allergiesData, setAllergiesData] = useState(initialHistoryData?.allergies || []);
  const [pastMedicalData, setPastMedicalData] = useState<OPIPHistPMHDto>(initialHistoryData?.pastMedicalHistory || ({} as OPIPHistPMHDto));
  const [pastSurgicalData, setPastSurgicalData] = useState<OPIPHistPSHDto>(initialHistoryData?.pastSurgicalHistory || ({} as OPIPHistPSHDto));
  const [pastMedicationData, setPastMedicationData] = useState(initialHistoryData?.pastMedication || []);
  const [reviewOfSystemData, setReviewOfSystemData] = useState<OPIPHistROSDto>(initialHistoryData?.reviewOfSystem || ({} as OPIPHistROSDto));
  const [socialHistoryData, setSocialHistoryData] = useState<OPIPHistSHDto>(initialHistoryData?.socialHistory || ({} as OPIPHistSHDto));
  const [familyHistoryData, setFamilyHistoryData] = useState<OPIPHistFHDto>(initialHistoryData?.familyHistory || ({} as OPIPHistFHDto));
  ...
  const handleHistoryDataChange = useCallback(
  ...
  return (
    ...
    >
      <Tabs value={activeHistoryTab} onChange={handleHistoryTabChange} aria-label="patient history tabs" textColor="primary" indicatorColor="primary">
        <Tab label="Allergies" />
