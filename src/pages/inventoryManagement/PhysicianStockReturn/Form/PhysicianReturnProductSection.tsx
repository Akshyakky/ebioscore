...
import { ProductStockReturnDetailDto } from "@/interfaces/InventoryManagement/ProductStockReturnDto";
...
interface ProductDetailsSectionProps {
  ...
  fields: FieldArrayWithId<PhysicianReturnFormData, "productStockReturnDetails", "id">[];
  append: UseFieldArrayAppend<PhysicianReturnFormData, "productStockReturnDetails">;
  remove: UseFieldArrayRemove;
  setValue: UseFormSetValue<PhysicianReturnFormData>;
  errors: FieldErrors<PhysicianReturnFormData>;
  isViewMode: boolean;
  showAlert: (type: string, message: string, severity: "success" | "error" | "warning" | "info") => void;
}

const PhysicianReturnProductSection: React.FC<ProductDetailsSectionProps> = ({ control, fields, append, remove, setValue, errors, isViewMode, showAlert }) => {
  ...
  const physicianID = useWatch({ control, name: "physicianID" });
  ...
  const handleProductSelect = useCallback(
    async (product: ProductListDto | null) => {
      ...
      try {
        ...
        setIsLoadingBatches(true);
        // In a real implementation, we would query available physician items
        // For now, we'll use the same mechanism as product returns
        const response = await billingService.getBatchNoProduct(product.productID, fromDeptID);
        if (response.success && response.data) {
          const batches = response.data;
          if (batches.length === 0) {
            showAlert("Warning", "No batches available for this product with the physician", "warning");
            clearTemporaryFields();
          } else {
            // In a real implementation, this would filter for physician items
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid size={{ sm: 12, md: 6 }}>
              <ProductSearch
                ...
                label={`Search Product for ${returnTypeCode === PhysicianReturnType.Physician ? "Physician Return" : "Inventory Adjustment"}`}
            ...
            <Grid size={{ sm: 6, md: 4 }}>
              <TextField
                ...
                size="small"
                fullWidth
                placeholder={`Reason for ${returnTypeCode === PhysicianReturnType.Physician ? "physician return" : "inventory adjustment"}`}
              />
            </Grid>
          </Grid>
        ...
        <Box sx={{ p: 2 }}>
          ...
          <Box sx={{ mt: 2, p: 2, bgcolor: "background.paper", borderRadius: 1, border: "1px solid", borderColor: "divider" }}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ sm: 3, xs: 3 }}>
                <Box sx={{ textAlign: "center" }}>
                  <Typography variant="h6" color={returnTypeCode === PhysicianReturnType.Physician ? "error.main" : "text.primary"}>
            ...
            <Box sx={{ mt: 2, display: "flex", flexWrap: "wrap", gap: 1 }}>
              ...
              {returnTypeCode && (
                <Chip label={returnTypeCode === PhysicianReturnType.Physician ? "Physician Return" : "Inventory Adjustment"} color="primary" size="small" icon={<InfoIcon />} />
              )}
            </Box>
          </Box>
        </Box>
...
export default PhysicianReturnProductSection;
