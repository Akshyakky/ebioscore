...
type PhysicianIssualFormData = z.infer<typeof schema>;
type PhysicianProductDetailWithId = ProductIssualDetailDto & {
  id: string;
  cgst?: number;
  sgst?: number;
  margin?: number;
...
interface PhysicianProductDetailsSectionProps {
  control: Control<PhysicianIssualFormData>;
  fields: FieldArrayWithId<PhysicianIssualFormData, "details", "id">[];
  append: UseFieldArrayAppend<PhysicianIssualFormData, "details">;
  remove: UseFieldArrayRemove;
  setValue: UseFormSetValue<PhysicianIssualFormData>;
  errors: FieldErrors<PhysicianIssualFormData>;
  isViewMode: boolean;
  showAlert: (type: string, message: string, severity: "success" | "error" | "warning" | "info") => void;
}

const PhysicianProductDetailsSection: React.FC<PhysicianProductDetailsSectionProps> = ({ control, fields, append, remove, setValue, errors, isViewMode, showAlert }) => {
    ...
    async (product: ProductListDto | null) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          showAlert("Warning", "Failed to fetch product batches for physician issual", "warning");
          clearTemporaryFields();
        }
      } catch (error) {
        showAlert("Error", "Failed to fetch product batches for physician issual", "error");
        clearTemporaryFields();
      } finally {
        setIsLoadingBatches(false);
      }
  ...
  const handleEditExistingProduct = useCallback(
    async (index: number) => {
      ...
      try {
        ...
        showAlert("Error", "Failed to load product data for editing physician issual", "error");
        clearTemporaryFields();
      } finally {
        setIsAddingProduct(false);
        setIsLoadingBatches(false);
  ...
  const dataGridRows: GridRowsProp<PhysicianProductDetailWithId> = useMemo(() => {
      ...
      renderCell: (params) => {
        ...
        return (
          <TextField
            ...
            error={!!errors.details?.[index]?.margin}
            sx={{
              width: "90px",
              "& .MuiInputBase-input": {
                cursor: "text",
      ...
      renderCell: (params) => {
        ...
        return (
          <Box sx={{ display: "flex", gap: 0.5 }}>
            ...
            <Tooltip title="Remove from Issual">
              <IconButton
                size="small"
                color="error"
                onClick={(e) => {
                  e.stopPropagation();
                  remove(index);
                  showAlert("Info", `Product "${params.row.productName}" removed from physician issual`, "info");
                }}
                sx={{
          ...
          <Typography variant="h6" gutterBottom sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            {isEditingExistingProduct ? (
              <>
                ...
                Edit Product for Physician Issual
              </>
            ) : (
              <>
                <AddIcon color="primary" />
                Add Products for Physician Issual
              </>
            )}
          </Typography>
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid size={{ sm: 12, md: 6 }}>
              <ProductSearch
                ...
                label="Search Product for Physician Issual"
                placeholder="Scan barcode or search product name for physician issual..."
              ...
              {isAddingProduct && (
                <Box sx={{ display: "flex", alignItems: "center", mt: 1 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Loading product data for physician issual...
                  </Typography>
                </Box>
              )}
            </Grid>
          ...
          {selectedProduct && (
            <Alert severity="success" sx={{ mt: 2 }}>
              <Typography variant="body2">
                <strong>Product Selected for Physician Issual:</strong> {selectedProduct.productName}
                <br />
                <strong>Product Code:</strong> {selectedProduct.productCode || "N/A"}
                <br />
                <strong>Status:</strong> Waiting for batch selection for physician issual...
              </Typography>
            </Alert>
          )}
        </Paper>
      ...
      <Paper elevation={1} sx={{ mb: 3 }}>
        <Box sx={{ p: 2 }}>
          <Typography variant="h6" gutterBottom>
            Physician Issual Products ({fields.length} items)
            {recConName && <Chip label={`Physician: ${recConName}`} size="small" color="success" variant="outlined" sx={{ ml: 2 }} />}
          </Typography>

          {fields.length === 0 ? (
            <Alert severity="info" sx={{ mb: 2 }}>
              No products added for physician issual yet. {!isViewMode && "Select products above to begin the issual process."}
            </Alert>
          ) : (
          ...
          {errors.details && (
            <Alert severity="error" sx={{ mt: 2 }}>
              At least one product detail is required for physician issual
            </Alert>
          )}

          <Box sx={{ mt: 2, p: 2, bgcolor: "background.paper", borderRadius: 1, border: "1px solid", borderColor: "divider" }}>
            <Typography variant="subtitle2" gutterBottom sx={{ fontWeight: 600 }}>
              Physician Issual Summary
            </Typography>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ sm: 2, xs: 6 }}>
                <Box sx={{ textAlign: "center" }}>
                  <Typography variant="h6" color={statistics.avgMargin > 20 ? "success.main" : statistics.avgMargin > 10 ? "warning.main" : "error.main"}>
                    {statistics.avgMargin.toFixed(2)}%
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    Avg Margin
                  </Typography>
            ...
            <Box sx={{ mt: 2, display: "flex", flexWrap: "wrap", gap: 1 }}>
              ...
              {statistics.totalProducts === 0 && <Chip label="No products added for physician issual yet" color="default" size="small" icon={<InfoIcon />} />}
              {statistics.totalProducts > 0 && (
                <Chip
                  label={`Issual Efficiency: ${statistics.totalRequestedQty > 0 ? ((statistics.totalIssuedQty / statistics.totalRequestedQty) * 100).toFixed(1) : 0}%`}
                  color="primary"
                  size="small"
                  icon={<InfoIcon />}
                />
...
export default PhysicianProductDetailsSection;
