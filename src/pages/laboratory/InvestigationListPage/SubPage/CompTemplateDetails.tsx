...
const CompTemplateDetails: React.FC<LCompTemplateDetailsProps> = ({ onUpdateTemplate, selectedComponent }) => {
  ...
  const [templateGroups, setTemplateGroups] = useState<AppModifyFieldDto[]>([]);
  ...
  const handleTemplateGroupSelect = (templateGroup: AppModifyFieldDto) => {
  ...
  const handleAddTemplate = () => {
    ...
    console.log("Component ID (compoID):", selectedComponent.compoID);
  ...
  const fetchTemplateGroups = async () => {
    ...
    try {
      ...
      const templateGroupFields = allFields.filter((field: AppModifyFieldDto) => field.amlField === "TEMPLATEGROUP" && field.rActiveYN === "Y");
  ...
  const handleEditTemplateGroup = async (group: AppModifyFieldDto) => {
    ...
    try {
      ...
      if (templateGroup) {
        setFormDataDialog({
          amlID: templateGroup.amlID,
          amlName: templateGroup.amlName,
          amlCode: templateGroup.amlCode,
          amlField: "TEMPLATEGROUP",
          defaultYN: templateGroup.defaultYN || "N",
          modifyYN: templateGroup.modifyYN || "N",
          rNotes: templateGroup.rNotes || "",
          rActiveYN: templateGroup.rActiveYN || "Y",
          compID: compID || 0,
          compCode: compCode || "",
          compName: compName || "",
          transferYN: templateGroup.transferYN || "Y",
        });
