...
export const getDeptUsersByDeptId = async (
  ...
  return apiService.get<OperationResult<any>>(
    `${apiCategory}/GetDeptUsersByDeptId/${deptID}`,
    getToken()
  );
};
export const saveDeptUser = async (
  deptUserDto: DeptUserDto
): Promise<OperationResult<DeptUserDto>> => {
  return apiService.post<OperationResult<DeptUserDto>>(
    `${apiCategory}/SaveDeptUser`,
    deptUserDto,
    getToken()
  );
};

export const updateDeptUserToggleStatus = async (
  deptUserID: number,
