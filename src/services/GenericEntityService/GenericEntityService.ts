...
export class GenericEntityService<T extends BaseDto> {
  ...
  async getAll(): Promise<T> {
    return this.apiService.get<T>(`${this.baseEndpoint}/GetAll`, this.getToken());
  }

  async getById(id: number): Promise<T> {
    return this.apiService.get<T>(`${this.baseEndpoint}/GetById/${id}`, this.getToken());
  }

  async save(entity: T): Promise<T> {
    return this.apiService.post<T>(`${this.baseEndpoint}/Save`, entity, this.getToken());
  }

  async updateActiveStatus(id: number, active: boolean): Promise<boolean> {
    return this.apiService.put<boolean>(`${this.baseEndpoint}/UpdateActiveStatus/${id}`, active, this.getToken());
  }
  ...
  async firstOrDefault(predicate: string): Promise<T | null> {
    return this.apiService.get<T | null>(`${this.baseEndpoint}/FirstOrDefault?predicate=${encodeURIComponent(predicate)}`, this.getToken());
  }

  async count(predicate?: string): Promise<number> {
    const params = predicate ? `?predicate=${encodeURIComponent(predicate)}` : "";
    return this.apiService.get<number>(`${this.baseEndpoint}/Count${params}`, this.getToken());
  }

  async any(predicate: string): Promise<boolean> {
    return this.apiService.get<boolean>(`${this.baseEndpoint}/Any?predicate=${encodeURIComponent(predicate)}`, this.getToken());
  }
  ...
  async bulkSave(entities: T[]): Promise<boolean> {
    return this.apiService.post<boolean>(`${this.baseEndpoint}/BulkSave`, entities, this.getToken());
  }

  async bulkUpdate(entities: T[]): Promise<boolean> {
    return this.apiService.put<boolean>(`${this.baseEndpoint}/BulkUpdate`, entities, this.getToken());
  }
}
