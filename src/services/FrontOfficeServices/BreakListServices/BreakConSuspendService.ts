...
export const getAllBreakConSuspends = async (): Promise<
  OperationResult<any[]>
> => {
  return commonApiService.get<OperationResult<any[]>>(
    "BreakConSuspend/GetAllBreakConSuspends",
    getToken()
  );
};

export const getBreakConSuspendById = async (
  bCSID: number
): Promise<OperationResult<any>> => {
  return commonApiService.get<OperationResult<any>>(
    `BreakConSuspend/GetBreakConSuspendById/${bCSID}`,
    getToken()
  );
};

export const updateBreakConSuspendActiveStatus = async (
  bCSID: number,
  isActive: boolean
): Promise<OperationResult<boolean>> => {
  return commonApiService.put<OperationResult<boolean>>(
    `BreakConSuspend/UpdateBreakConSuspendActiveStatus/${bCSID}`,
    isActive,
    getToken()
  );
};
