  ...
  try {
    ...
    if (response.data && response.data.success !== undefined) {
      if (!response.data.success) {
        throw new Error(
          response.data.errorMessage || "Failed to save reason list."
        );
      }
...
const getAllReasonLists = async (
  ...
  try {
    ...
    const response = await axios.get<OperationResult<ReasonListData[]>>(url, {
      headers,
    });
    ...
    if (response.data && response.data.success !== undefined) {
      if (!response.data.success) {
        throw new Error(
          response.data.errorMessage || "Failed to fetch reason lists."
        );
      }
      return response.data;
    } else {
