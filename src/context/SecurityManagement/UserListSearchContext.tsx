...
interface UserListSearchContextProps {
  ...
  updateUserStatus: (appID: number, status: string) => void;
}
...
export const UserListSearchProvider = ({
  children,
}: UserListSearchProviderProps) => {
  ...
  const fetchAllUsers = async () => {
    ...
    setLoading(true);
    try {
      const result = await UserListService.getAllUsers(token); // Use getAllUsers instead of getActiveWorkingUsers
  ...
  const performSearch = async (searchTerm: string): Promise<void> => {
    if (searchTerm.trim() === "") {
      ...
      const filteredResults = allUsers.filter(
        (user) =>
          user.appUserName.toLowerCase().includes(searchTerm.toLowerCase()) ||
          user.appGeneralCode.toLowerCase().includes(searchTerm.toLowerCase())
      );
      setSearchResults(filteredResults);
    }
  };

  const updateUserStatus = (appID: number, status: string) => {
    const updatedUsers = allUsers.map((user) =>
      user.appID === appID ? { ...user, rActiveYN: status } : user
    );
